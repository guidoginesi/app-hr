-- Migration: segundo PDF de recibo para relación de dependencia

ALTER TABLE public.payroll_payslips
  ADD COLUMN IF NOT EXISTS pdf2_storage_path text,
  ADD COLUMN IF NOT EXISTS pdf2_filename text,
  ADD COLUMN IF NOT EXISTS pdf2_uploaded_at timestamptz,
  ADD COLUMN IF NOT EXISTS pdf2_uploaded_by uuid REFERENCES auth.users(id) ON DELETE SET NULL;
...
COMMENT ON VIEW public.payroll_settlements_with_details IS
  'Settlements con detalles de período, empleado, breakdown, recibos (2 PDF) y factura.';
