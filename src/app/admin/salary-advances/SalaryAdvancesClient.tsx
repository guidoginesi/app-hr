  ...
  const renderActions = (a: SalaryAdvanceWithDetails) => {
    ...
    return (
      <div className="space-y-4 rounded-lg border border-[var(--border)] bg-muted p-4">
        ...
        {['settled', 'rejected', 'blocked'].includes(a.status) ? (
          a.rejection_reason ? <p className="text-sm text-muted-foreground"><span className="font-medium text-foreground">Motivo:</span> {a.rejection_reason}</p> : <p className="text-sm text-muted-foreground">Sin acciones disponibles.</p>
        ) : (
