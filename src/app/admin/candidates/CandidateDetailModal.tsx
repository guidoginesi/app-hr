		...
		<Modal isOpen={true} onClose={onClose} title="" maxWidth="max-w-6xl">
			...
			<div className="mb-6">
				<div className="flex items-start justify-between">
					<div>
						<h2 className="text-base font-semibold text-foreground">{displayCandidate.name}</h2>
						{mainApplication && (
				...
				<div className="rounded-lg border border-[var(--border)] bg-white p-4">
					...
					<div className="space-y-2">
						<div className="flex items-center gap-2">
							...
							<a href={`mailto:${displayCandidate.email}`} className="text-sm text-secondary-foreground hover:text-foreground">
						...
						{displayCandidate.phone && (
							<div className="flex items-center gap-2">
								...
								<a href={`tel:${displayCandidate.phone}`} className="text-sm text-secondary-foreground hover:text-foreground">
									{displayCandidate.phone}
								</a>
							</div>
						)}
				...
				<div className="mb-6">
					...
					{mainApplication.final_outcome === FinalOutcome.HIRED && (
						<div className="mt-4 flex items-center gap-3 rounded-lg border border-success/20 bg-success-subtle p-4">
							...
							<Button type="button" onClick={() => setShowConvertModal(true)}>
			...
			{/* Tabs */}
			<div className="mb-4">
				<TabNav<'history' | 'cv' | 'notes' | 'rating'>
					aria-label="Secciones del candidato"
					value={activeTab as 'history' | 'cv' | 'notes' | 'rating'}
			...
			<div className="min-h-[300px]">
				{activeTab === 'history' && (
					<div className="space-y-3">
						{(() => {
							// Combinar historial de etapas, notas del reclutador, emails enviados y calificaciones
							...
							const ratings = mainApplication?.rating_history || [];
							...
							const combinedHistory = [
								...
								...ratings.map(r => ({ type: 'rating' as const, data: r, date: r.created_at }))
							];
							...
							return combinedHistory.map((item, index) => {
										...
										<div className="flex items-start justify-between gap-4">
											<div className="flex-1 space-y-2">
												...
												{entry.notes && entry.notes.trim() !== '' && (
													<div className="mt-3 rounded-lg bg-muted border border-[var(--border)] px-3 py-2">
														<div className="flex items-start gap-2">
															...
															<div className="flex-1">
																<p className="text-xs font-semibold text-secondary-foreground mb-1">Notas:</p>
																<div className="text-sm text-secondary-foreground leading-relaxed space-y-0.5">
								...
								return (
									<div key={`note-${note.id}`} className="rounded-lg border border-[var(--border)] bg-muted p-4">
										<div className="flex items-start gap-3">
											...
											<div className="flex-1 space-y-2">
												{/* Título y fecha en la misma línea */}
												<div className="flex items-center justify-between gap-4">
													<span className="text-sm font-semibold text-foreground">Nota del Reclutador</span>
												...
												<div className="mt-1 text-sm text-secondary-foreground leading-relaxed whitespace-pre-wrap">
													{note.content}
												</div>
											</div>
										</div>
								...
								return (
									<div key={`email-${email.id}`} className="rounded-lg border border-[var(--border)] bg-white p-4">
										<div className="flex items-start gap-3">
											<div className="flex-shrink-0 mt-0.5">
											...
											<div className="flex-1 space-y-2">
												...
												<div className="flex items-center justify-between gap-4">
													<div className="flex items-center gap-2">
														<span className="text-sm font-semibold text-foreground">Email Enviado</span>
														{email.error && (
												...
												<div className="mt-2 space-y-1">
													...
													<div>
														<p className={`text-xs text-muted-foreground leading-snug whitespace-pre-wrap ${!expandedEmails.has(email.id) ? 'line-clamp-3' : ''}`}>
															{email.body}
														</p>
								...
								const rating = item.data as RatingHistory;
								const ratingDate = new Date(rating.created_at);
								const dateStr = ratingDate.toLocaleDateString('es-AR', {
									day: 'numeric',
									month: 'long',
									year: 'numeric'
								});
								const timeStr = ratingDate.toLocaleTimeString('es-AR', {
									hour: '2-digit',
									minute: '2-digit'
								});
								
								return (
									<div key={`rating-${rating.id}`} className="rounded-lg border border-[var(--border)] bg-white p-4">
										<div className="flex items-start gap-3">
											<div className="flex-shrink-0 mt-0.5">
												<div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center">
											...
											<div className="flex-1 space-y-2">
												...
												<div className="flex items-center justify-between gap-4">
													<div className="flex items-center gap-2">
														...
														<span className="text-sm font-bold text-foreground">({rating.rating}/5)</span>
													</div>
													<div className="flex items-center gap-1.5 text-xs text-muted-foreground">
														<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
															<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
					...
					<div className="space-y-4">
						<div className="rounded-lg border border-[var(--border)] bg-white p-4">
							...
							<textarea
								...
								className="w-full rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-brand focus:outline-none focus:ring-1 focus:ring-ring disabled:opacity-50 disabled:cursor-not-allowed"
										...
										<button
											...
											onClick={async () => {
												...
												try {
													...
													// Limpiar nota y recargar datos del candidato
													setRatingNote('');
													const candidateRes = await fetch(`/api/admin/candidates/${candidate.id}`);
													if (candidateRes.ok) {
														const updatedCandidate = await candidateRes.json();
							...
							{mainApplication?.recruiter_rating && (
								<div className="text-center">
									<p className="text-sm font-medium text-secondary-foreground">
										Calificación actual: <span className="text-lg font-bold text-foreground">{mainApplication.recruiter_rating}</span> / 5
									</p>
									<button
										type="button"
										onClick={async () => {
